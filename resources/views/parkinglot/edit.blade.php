	...
	<div class="row mb-4 align-items-center h-custom">
		<div class="panel panel-default h-custom">
			<div class="panel-heading text-center">
				<b class="text-info" style="font-size: 18px;">Edit Tempat Parkir</b>
			</div>
			<div class="panel-body h-100">
				<div class="row h-100">
					<div class="col-md-6 col-md-offset-3 h-100">
						<form role="form" class="h-100" action="{{ url('parkinglot/'.$parkinglot->id) }}" method="POST" id="form-tambah" enctype="multipart/form-data">
							@method('PUT')
							@csrf
							<input type="hidden" name="id">
							<div class="form-group">
								<label>Nama Tempat</label>
								<input class="form-control" placeholder="Masukkan Nama Tempat" name="nama_tempat" data-validation="length alphanumeric required" data-validation-length="min4" data-validation-allowing=" " required="" value="">
							</div>
							<div class="form-group">
								<label>Lokasi</label>
...
<script>
 ...
 <script type="text/javascript">
 	...
 	function setData(data){
 		...
 		$('[name=long]').val(data.lon);
 		$('[name=lat]').val(data.lat);
 		$('[name=kapasitas]').val(data.kapasitas);
 		$('[name=biaya]').val(data.biaya);
 		$('[name=biaya_akumulasi]').val(data.biaya_akumulasi);
 		$('[name=gerbang]').val(data.gerbang);
